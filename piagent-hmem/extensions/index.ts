...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("hmem", {
		...
		handler: async (args: string, ctx: ExtensionContext) => {
			...
			switch (cmd) {
				...
				default:
					ctx.ui.notify(
						`Unknown subcommand: ${cmd}. Try /hmem help`,
						"warning",
					);
			}
		},
	...
	registerTool(pi, "hmem_write", {
		...
		parameters: Type.Object({
			...
			memory_type: Type.Optional(
				Type.String({
					description: "Type: observation (default) or experience. insight/mental_model not allowed — they're auto-generated by hmem_reflect.",
				}),
			),
		...
		async execute(_id, params, _signal, _update, _ctx) {
			...
			const result = await Tools.handleWrite(
				c,
	...
	registerTool(pi, "hmem_get", {
		...
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleGet(
				c,
				params as Record<string, unknown>,
			);
			return {
	...
	registerTool(pi, "hmem_stats", {
		...
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleStats(
				c,
				params as Record<string, unknown>,
			);
			return {
	...
	registerTool(pi, "hmem_graph", {
		...
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleGraph(
				c,
				params as Record<string, unknown>,
			);
			return {
