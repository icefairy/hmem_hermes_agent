...
export default function (pi: ExtensionAPI) {
		...
		handler: async (args: string, ctx: ExtensionContext) => {
			...
			switch (cmd) {
				case "help": {
					ctx.ui.notify(
						[
							...
							"  /hmem doc list [ns] — list documents in a knowledge base",
						"  /hmem search knowledge:<q> — search knowledge bases",
						].join("\n"),
						"info",
					);
					return;
				...
				case "kb": {
					...
					if (sub === "list") {
						const result = await Tools.handleKbList(c);
						ctx.ui.notify(result, "info");
					} else if (sub === "create") {
						const result = await Tools.handleKbCreate(
							c,
							parts[2] ? { namespace: parts[2] } : {},
						);
						ctx.ui.notify(result, "info");
					} else if (sub === "delete") {
						const result = await Tools.handleKbDelete(
							c,
							parts[2] ? { namespace: parts[2] } : {},
						);
						ctx.ui.notify(result, "info");
					} else if (sub === "categories") {
						const result = await Tools.handleKbCategories(c, {});
						ctx.ui.notify(result, "info");
					} else {
						ctx.ui.notify(
							"Usage: /hmem kb list | create <ns> | delete <ns> | categories",
							"info",
						);
					}
					return;
				...
				case "doc": {
					const sub = parts[1]?.toLowerCase();
					if (sub === "list") {
						const result = await Tools.handleDocList(c, {});
						ctx.ui.notify(result, "info");
					} else if (sub === "get") {
						const result = await Tools.handleDocGet(
							c,
							parts[2] ? { doc_id: parts[2] } : {},
						);
						ctx.ui.notify(result, "info");
					} else if (sub === "delete") {
						const result = await Tools.handleDocDelete(
							c,
							parts[2] ? { doc_id: parts[2] } : {},
						);
						ctx.ui.notify(result, "info");
						...
						ctx.ui.notify(
							"Usage: /hmem doc list | get <doc_id> | delete <doc_id>",
						"info",
						);
					}
					return;
	...
	pi.on("before_agent_start", async (event: any) => {
		...
		const contextBlock = [
			...
			`By type: ${Object.entries(s.by_type ?? {})
				.map(([k, v]) => `${k}:${v}`)
				.join(", ")}`,
							"Use tools: hmem_write, hmem_search, hmem_list, hmem_get, hmem_delete, hmem_stats, hmem_reflect, hmem_models, hmem_graph, hmem_namespaces, hmem_doc_import, hmem_doc_list, hmem_doc_get, hmem_doc_delete, hmem_kb_put, hmem_kb_query, hmem_kb_categories, hmem_kb_create, hmem_kb_list, hmem_kb_delete.",
	...
	registerTool(pi, "hmem_doc_import", {
		description:
			"Import a plain-text document into a knowledge base — auto-chunked and vectorized on the server. Overwrites same doc_id. Knowledge entries are excluded from time-decay and auto-reflection for fidelity.",
		parameters: Type.Object({
			content: Type.String({ description: "Full document text to import" }),
			doc_id: Type.Optional(
				Type.String({
					description:
						"Stable document id (auto-generated from title if omitted)",
				}),
			),
			title: Type.Optional(Type.String({ description: "Document title" })),
			uri: Type.Optional(Type.String({ description: "Source URI (optional)" })),
			category: Type.Optional(
				Type.String({ description: "Document category (optional)" }),
			),
			tags: Type.Optional(
				Type.Array(Type.String(), { description: "Tags (optional)" }),
			),
			chunk_size: Type.Optional(
				Type.Integer({ description: "Chunk size in tokens (default 800)" }),
			),
			overlap: Type.Optional(
				Type.Integer({ description: "Chunk overlap in tokens (default 80)" }),
			),
			namespace: Type.Optional(
				Type.String({ description: "Knowledge base namespace (default: default)" }),
			),
		}),
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleDocImport(
				c,
				params as Record<string, unknown>,
			);
			return {
	...
	registerTool(pi, "hmem_doc_list", {
		description: "List all documents in a knowledge namespace (with chunk counts).",
		parameters: Type.Object({
			namespace: Type.Optional(
				Type.String({ description: "Knowledge namespace (default ...)" }),
			),
		}),
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleDocList(
				c,
				params as Record<string, unknown>,
			);
			return {
	...
	registerTool(pi, "hmem_doc_delete", {
		description:
			"Cascade-delete a document and all its chunks by doc_id.",
		parameters: Type.Object({
	...
	registerTool(pi, "hmem_kb_put", {
		description:
			"Add a single knowledge entry directly to a knowledge base (optionally tagged with category/doc_id). For whole documents use hmem_doc_import instead.",
		parameters: Type.Object({
			content: Type.String({ description: "Knowledge content" }),
			doc_id: Type.Optional(
				Type.String({ description: "Optional doc_id to group under" }),
			),
			title: Type.Optional(Type.String({ description: "Entry title" })),
			category: Type.Optional(
				Type.String({ description: "Category (optional)" }),
			),
			tags: Type.Optional(
				Type.Array(Type.String(), { description: "Tags (optional)" }),
			),
		...
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleKbPut(
				c,
				params as Record<string, unknown>,
			);
			return {
	...
	registerTool(pi, "hmem_kb_query", {
		description:
			"List knowledge entries in a knowledge base, optionally filtered by category / doc_id / tags.",
		parameters: Type.Object({
			namespace: Type.Optional(
				Type.String({ description: "Knowledge base (default ...)" }),
			),
			limit: Type.Optional(
				Type.Integer({ description: "Max results (default 50, max 200)" }),
			),
			offset: Type.Optional(Type.Integer({ description: "Pagination offset" })),
			category: Type.Optional(
			...
			doc_id: Type.Optional(
				Type.String({ description: "Filter by doc_id" }),
			),
			tags: Type.Optional(Type.String({ description: "Filter by tag" })),
		}),
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleKbQuery(
				c,
				params as Record<string, unknown>,
			);
			return {
	...
	registerTool(pi, "hmem_kb_categories", {
		description:
			"Knowledge base category summary: entries/documents/tags per category.",
		parameters: Type.Object({
			namespace: Type.Optional(
				Type.String({ description: "Knowledge base (default ...)" }),
			),
		...
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleKbCategories(
				c,
				params as Record<string, unknown>,
			);
			return {
	...
	registerTool(pi, "hmem_kb_create", {
		description:
			"Create/activate a knowledge base (independent namespace). Idempotent — safe to call repeatedly.",
		parameters: Type.Object({
	...
	registerTool(pi, "hmem_kb_list", {
		description:
			"List all knowledge bases with entry/doc/category counts.",
		parameters: Type.Object({}),
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
			const result = await Tools.handleKbList(c);
	...
	registerTool(pi, "hmem_kb_delete", {
		description:
			"Delete a knowledge base (its namespace). Default keeps the db file; pass hard=true to remove it.",
		parameters: Type.Object({
			namespace: Type.String({ description: "Knowledge base name to delete" }),
			hard: Type.Optional(
				Type.Boolean({ description: "Also remove the db file (default false)" }),
			),
		}),
		async execute(_id, params, _signal, _update, _ctx) {
			const c = getClient(_ctx.cwd);
